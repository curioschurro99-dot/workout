...
import {
  Dumbbell, Calendar, Library, TrendingUp, BookOpen,
  Sun, Apple, LogOut, Plus, X, Check, ChevronLeft, ChevronRight,
  Play, Pause, RotateCcw, CheckCircle,
} from "lucide-react";
import { AuthProvider, useAuth } from "./lib/auth-context";
import { useLocalStorage } from "./lib/store";
import {
  type Exercise, type Goal, type Note, type Program, type WorkoutState,
  seedState, uid, SHAPES, POSES,
} from "./lib/workout-data";
import {
  type FoodItem, type MealSuggestion,
  type NutritionState, seedNutritionState,
      ...
      <div className="min-h-screen bg-bg flex items-center justify-center p-4">
        <div className="w-full max-w-sm">
          <div className="text-center mb-8">
            <div className="text-2xl font-['Oswald'] uppercase tracking-wider text-bone">FieldNotes</div>
            <div className="text-xs uppercase tracking-[0.08em] text-bone-dim mt-1">Training Log</div>
          </div>
          {authForm === "login" ? (
    ...
    <div className="min-h-screen bg-bg flex flex-col lg:flex-row">
      ...
      <div className="lg:w-[220px] bg-surface border-b lg:border-b-0 lg:border-r border-line flex lg:flex-col shrink-0 overflow-x-auto">
        <div className="hidden lg:block px-5 pb-5 border-b border-line mb-4">
          <div className="text-[22px] font-['Oswald'] uppercase tracking-wider text-bone leading-tight">FieldNotes</div>
          <div className="text-[11px] uppercase tracking-[0.08em] text-bone-dim mt-1">Training Log</div>
        </div>
        {[
...
/* ==================== TODAY VIEW ==================== */
function TodayView({
  state, updateState, openModal: _openModal, closeModal: _closeModal,
}: {
  state: WorkoutState;
    ...
    <>
      ...
      <div className="bg-surface border border-line rounded-[10px] p-5">
        ...
        {prog && (!day || day.exercises.length === 0) && (
          <>
            <h3 className="text-brass text-sm uppercase tracking-[0.05em] mb-2">{day?.focus || "Rest Day"}</h3>
            <div className="text-bone-dim text-sm py-3">Rest day. Recovery is part of the plan.</div>
          </>
        )}
        {prog && day && day.exercises.length > 0 && (
          <>
            ...
            {day.exercises.map((pe, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-3.5 py-3.5 border-b border-line last:border-b-0">
                  <button
...
function TimerCard() {
  ...
  return (
    <div className="bg-surface border border-line rounded-[10px] p-5 mt-4">
      <div className="text-xs uppercase tracking-[0.15em] text-bone-dim text-center mb-1">Interval Timer</div>
      <div className="flex justify-center gap-4 mb-2">
        <div className="w-[90px]">
          <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Work (s)</label>
        ...
        <div className="w-[90px]">
          <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Rest (s)</label>
          <input type="number" value={rest} onChange={(e) => setRest(Number(e.target.value))}
            className="w-full bg-bg border border-line text-bone px-2.5 py-2 rounded-md text-sm text-center" />
        </div>
        <div className="w-[90px]">
          <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Rounds</label>
      ...
      <div className="text-xs uppercase tracking-[0.15em] text-bone-dim text-center mb-1">
        {phase === "idle" ? "Ready" : `${phase === "work" ? "Work" : "Rest"} — Round ${round}/${rounds}`}
      </div>
      <div className={`font-['IBM_Plex_Mono'] text-5xl lg:text-6xl font-semibold text-center py-4 tracking-[0.02em]
        ${phase === "rest" ? "text-moss" : "text-brass"}`}>
        {fmt(remaining)}
      </div>
      <div className="flex gap-2.5 justify-center mt-3.5">
        <button onClick={start} className="bg-brass text-[#1a1408] border-none px-4 py-2 rounded-md font-semibold text-xs uppercase tracking-[0.03em] hover:bg-[#d69a4e]">
          <Play className="w-3.5 h-3.5 inline mr-1" /> Start
        </button>
        <button onClick={pause} className="bg-transparent text-bone border border-line px-4 py-2 rounded-md font-semibold text-xs uppercase tracking-[0.03em] hover:border-brass hover:text-brass">
          <Pause className="w-3.5 h-3.5 inline mr-1" /> Pause
        </button>
        <button onClick={reset} className="bg-transparent text-bone border border-line px-4 py-2 rounded-md font-semibold text-xs uppercase tracking-[0.03em] hover:border-brass hover:text-brass">
...
function ProgramsView({
  state, updateState, openModal, closeModal,
}: {
  state: WorkoutState;
  updateState: (fn: (prev: WorkoutState) => WorkoutState) => void;
  openModal: (content: React.ReactNode) => void;
  ...
  return (
    <>
      <div className="flex justify-between items-end flex-wrap gap-3 mb-7">
        <div>
          <h1 className="text-[30px] font-['Oswald'] uppercase tracking-wider text-bone">Programs</h1>
          <div className="text-sm text-bone-dim mt-1.5 max-w-[520px]">
            ...
            <div className="flex justify-between items-start gap-3">
              ...
              <div className="flex gap-2 shrink-0">
                {p.id === state.activeProgramId ? (
                  <span className="text-[10px] uppercase tracking-[0.05em] px-2 py-1 rounded-full border border-brass text-brass">Active</span>
                ) : (
                  <button
                    onClick={(e) => { e.stopPropagation(); updateState((s) => ({ ...s, activeProgramId: p.id, activeDayIndex: 0 })); }}
            ...
            <div className="mt-3.5 space-y-2.5">
              {p.days.map((d, i) => (
                <div key={i} className="border border-line rounded-lg p-3.5">
                  <h3 className="text-sm text-brass mb-2">{d.label} &mdash; {d.focus}</h3>
                  <div className="text-sm text-bone-dim space-y-1">
                    {d.exercises.length > 0
                      ? d.exercises.map((pe, j) => {
                          const ex = state.exercises.find((e) => e.id === pe.exId);
                          return ex ? <div key={j}>{ex.name} &mdash; {pe.sets}&times;{pe.reps}</div> : null;
                        })
                      : <div>Rest</div>}
                  </div>
                </div>
...
function NewProgramForm({ onSave }: { onSave: (name: string, desc: string) => void }) {
  ...
  return (
    <>
      ...
      <div className="mb-3">
        <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Program Name</label>
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. My Custom Split"
          className="w-full bg-bg border border-line text-bone px-2.5 py-2 rounded-md text-sm" />
      </div>
      <div className="mb-4">
        <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Description</label>
        <textarea value={desc} onChange={(e) => setDesc(e.target.value)} rows={2} placeholder="What is this program for?"
          className="w-full bg-bg border border-line text-bone px-2.5 py-2 rounded-md text-sm" />
      </div>
      <button onClick={() => { if (!name.trim()) return; onSave(name.trim(), desc.trim()); }}
...
function AddDayForm({
  program, exercises, onSave,
}: {
  program: Program;
  exercises: Exercise[];
  onSave: (label: string, focus: string, exercises: { exId: string; sets: number; reps: number }[]) => void;
}) {
  ...
  return (
    <>
      ...
      <div className="mb-3">
        <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Day Label</label>
        <input value={label} onChange={(e) => setLabel(e.target.value)}
          className="w-full bg-bg border border-line text-bone px-2.5 py-2 rounded-md text-sm" />
      </div>
      <div className="mb-3">
        <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Focus</label>
        <input value={focus} onChange={(e) => setFocus(e.target.value)} placeholder="e.g. Lower Body Strength"
          className="w-full bg-bg border border-line text-bone px-2.5 py-2 rounded-md text-sm" />
      </div>
      <div className="mb-3">
        <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Exercises</label>
        <div className="max-h-48 overflow-y-auto space-y-1">
          {exercises.map((ex) => (
            <label key={ex.id} className="flex items-center gap-2 text-sm text-bone-dim cursor-pointer">
...
function LibraryView({
  ...
  return (
    <>
      <div className="flex justify-between items-end flex-wrap gap-3 mb-7">
        <div>
          <h1 className="text-[30px] font-['Oswald'] uppercase tracking-wider text-bone">Library</h1>
          <div className="text-sm text-bone-dim mt-1.5 max-w-[520px]">Every exercise available to add into a program.</div>
        </div>
        <button
          onClick={() => {
            openModal(
      ...
      <div className="bg-surface border border-line rounded-[10px] p-5">
        {filtered.map((ex) => (
          <div key={ex.id} className="flex justify-between items-center gap-3 py-3.5 border-b border-line last:border-b-0">
            <div className="min-w-0">
              <div className="font-semibold text-sm text-bone">
                {ex.name}
                <span className={`text-[10px] uppercase tracking-[0.05em] px-2 py-0.5 rounded-full border ml-2
        ...
        {filtered.length === 0 && (
          <div className="text-center text-bone-dim text-sm py-5">No exercises in this category.</div>
        )}
      </div>
    </>
  );
...
function NewExerciseForm({ onSave }: { onSave: (ex: Exercise) => void }) {
  ...
  return (
    <>
      ...
      <div className="mb-3">
        <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Name</label>
        <input value={name} onChange={(e) => setName(e.target.value)}
          className="w-full bg-bg border border-line text-bone px-2.5 py-2 rounded-md text-sm" />
      </div>
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div>
          <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Category</label>
        ...
        <div>
          <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Muscle / Focus</label>
          <input value={muscle} onChange={(e) => setMuscle(e.target.value)} placeholder="e.g. Legs"
            className="w-full bg-bg border border-line text-bone px-2.5 py-2 rounded-md text-sm" />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div>
          <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Default Sets</label>
          <input type="number" value={sets} onChange={(e) => setSets(Number(e.target.value))}
            className="w-full bg-bg border border-line text-bone px-2.5 py-2 rounded-md text-sm" />
        </div>
        <div>
          <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Default Reps</label>
          <input type="number" value={reps} onChange={(e) => setReps(Number(e.target.value))}
            className="w-full bg-bg border border-line text-bone px-2.5 py-2 rounded-md text-sm" />
        </div>
      </div>
      <div className="mb-4">
        <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Note (optional)</label>
  ...
  return (
    <>
      <div className="mb-7">
        ...
        <div className="text-sm text-bone-dim mt-1.5">Goals, streaks, and your session history.</div>
      </div>
      ...
      <div className="grid grid-cols-3 max-lg:grid-cols-1 gap-4 mb-4">
        {[
          ...
          <div key={i} className="bg-surface border border-line rounded-[10px] p-4 text-center">
            ...
            <div className="text-[11px] uppercase tracking-[0.05em] text-bone-dim mt-1">{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-surface border border-line rounded-[10px] p-5 mb-4">
          ...
          <button
            onClick={() => {
              openModal(
                <EditGoalForm
                  goal={g}
                  onSave={(goal) => { updateState((s) => ({ ...s, goal })); closeModal(); }}
                />
              );
            }}
        ...
        <div className="mt-2.5">
          <div className="text-sm text-bone-dim mb-2">
            {g.targetWeight && <>Target weight: <b className="text-bone">{g.targetWeight}kg</b> &middot; </>}
          ...
          <div className="h-2 bg-bg rounded-full overflow-hidden border border-line">
            <div className="h-full bg-gradient-to-r from-brass-dim to-brass transition-all" style={{ width: `${pct}%` }} />
          </div>
          <div className="text-[11px] text-bone-dim mt-1">{weekCount}/{g.weeklyTarget} sessions this week</div>
        </div>
      </div>
        ...
        {logs.length === 0 ? (
          ...
          <div>
            {logs.slice().reverse().slice(0, 30).map((l, i) => (
              <div key={i} className="flex items-center gap-3.5 py-3.5 border-b border-line last:border-b-0">
                <div className="min-w-0">
                  <div className="font-semibold text-sm text-bone">{l.dayLabel} &mdash; {l.focus}</div>
                  <div className="text-xs text-bone-dim mt-0.5">{l.date} &middot; {l.exCount} exercises</div>
                </div>
              </div>
            ))}
          </div>
...
function EditGoalForm({ goal, onSave }: { goal: Goal; onSave: (goal: Goal) => void }) {
  ...
  return (
    <>
      ...
      <div className="mb-3">
        <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Target Weight (kg, optional)</label>
        <input type="number" value={targetWeight} onChange={(e) => setTargetWeight(e.target.value)}
          className="w-full bg-bg border border-line text-bone px-2.5 py-2 rounded-md text-sm" />
      </div>
      <div className="mb-3">
        <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Weekly Session Target</label>
        <input type="number" value={weeklyTarget} onChange={(e) => setWeeklyTarget(Number(e.target.value))}
          className="w-full bg-bg border border-line text-bone px-2.5 py-2 rounded-md text-sm" />
      </div>
      <div className="mb-4">
        <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Notes</label>
        <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3}
          className="w-full bg-bg border border-line text-bone px-2.5 py-2 rounded-md text-sm" />
      </div>
      <button onClick={() => onSave({ targetWeight, weeklyTarget, notes, startDate: goal.startDate })}
        className="bg-brass text-[#1a1408] border-none px-4 py-2 rounded-md font-semibold text-xs uppercase tracking-[0.03em] hover:bg-[#d69a4e]">
        Save Goal
      </button>
    </>
...
function KnowledgeView({
  ...
  return (
    <>
      <div className="mb-7">
        <h1 className="text-[30px] font-['Oswald'] uppercase tracking-wider text-bone">Knowledge</h1>
        <div className="text-sm text-bone-dim mt-1.5">Reference notes &mdash; why the plan is built this way.</div>
      </div>
      ...
      <div className="bg-surface border border-line rounded-[10px] p-5 space-y-4 mb-4">
        <div>
          ...
          <p className="text-sm text-bone-dim leading-relaxed">
            You don't need to build muscle first and lose fat later. Training for strength while eating in a modest calorie deficit lets a beginner lose fat and build muscle at the same time &mdash; the response to new training stimulus is fastest in the first few months.
          </p>
        </div>
        <div>
          <h3 className="text-sm text-brass mb-2">Strength before pure cardio</h3>
          <p className="text-sm text-bone-dim leading-relaxed">
            HIIT burns calories short-term. Strength training builds the muscle that raises resting metabolism long-term and shapes the body. Starting split: 4 strength days, 1&ndash;2 light cardio/HIIT days, rest days built in.
          </p>
        </div>
        <div>
          <h3 className="text-sm text-brass mb-2">Protein &amp; calories matter more than the workout</h3>
          <p className="text-sm text-bone-dim leading-relaxed">
            Fat loss needs a calorie deficit; muscle building needs enough protein &mdash; roughly 1.2&ndash;1.6g per kg of bodyweight is a common target &mdash; plus enough total food to fuel the sessions. Moderate deficit, adequate protein, is the combination that lets both happen together.
          </p>
        </div>
        <div>
          <h3 className="text-sm text-brass mb-2">Choosing a starting dumbbell weight</h3>
          <p className="text-sm text-bone-dim leading-relaxed">
            The right weight is one where the last 2&ndash;3 reps of a set feel hard but form stays clean. Breezing through with no fatigue: go up. Form breaking down before the set ends: go lighter. Starting points for a smaller frame &mdash; goblet squats 4&ndash;6kg, rows 3&ndash;4kg, shoulder press 2&ndash;3kg, curls 2&ndash;3kg per hand.
          </p>
        </div>
        <div>
          <h3 className="text-sm text-brass mb-2">Progression rule</h3>
          <p className="text-sm text-bone-dim leading-relaxed">
            When a rep range feels easy for two sessions in a row, add weight or reps. Consistency across 8&ndash;12 weeks matters more than intensity in week one.
          </p>
        </div>
      </div>
            ...
            {state.myNotes.map((n, i) => (
              <div key={i}>
                <div className="flex justify-between items-center">
                  ...
                  <button
                    onClick={() => updateState((s) => ({ ...s, myNotes: s.myNotes.filter((_, j) => j !== i) }))}
...
function AddNoteForm({ onSave }: { onSave: (note: Note) => void }) {
  ...
  return (
    <>
      ...
      <div className="mb-3">
        <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Title</label>
        <input value={title} onChange={(e) => setTitle(e.target.value)}
          className="w-full bg-bg border border-line text-bone px-2.5 py-2 rounded-md text-sm" />
      </div>
      <div className="mb-4">
        <label className="text-[11px] uppercase tracking-[0.05em] text-bone-dim block mb-1.5">Note</label>
        <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={4}
          className="w-full bg-bg border border-line text-bone px-2.5 py-2 rounded-md text-sm" />
      </div>
      <button onClick={() => { if (!body.trim()) return; onSave({ title: title.trim(), body: body.trim() }); }}
        className="bg-brass text-[#1a1408] border-none px-4 py-2 rounded-md font-semibold text-xs uppercase tracking-[0.03em] hover:bg-[#d69a4e]">
        Save Note
      </button>
    </>
...
/* ==================== YOGA VIEW ==================== */
function YogaView({
  openModal, closeModal: _closeModal,
}: {
  openModal: (content: React.ReactNode) => void;
  closeModal: () => void;
}) {
  ...
  const goToPose = useCallback((idx: number) => {
  ...
  if (inSequence) {
    ...
    return (
      <>
        <div className="mb-7">
          <h1 className="text-[30px] font-['Oswald'] uppercase tracking-wider text-bone">Yoga Flow</h1>
        </div>

        {/* Video panel */}
        <div className="bg-surface border border-line rounded-[10px] p-4 mb-4">
          <div className="flex justify-between items-center mb-2.5">
            <h3 className="text-sm text-brass uppercase tracking-[0.05em]">Follow a Real Instructor (optional)</h3>
          ...
          <div className="flex gap-2">
            <input value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} placeholder="Paste a YouTube link..."
        ...
        <div className="flex justify-center items-center gap-4 mb-3.5">
          <label className="flex items-center gap-2 text-xs text-bone-dim uppercase tracking-[0.04em] cursor-pointer">
            <input type="checkbox" checked={voiceCoach} onChange={(e) => setVoiceCoach(e.target.checked)} className="w-4 h-4 accent-brass" />
        ...
        {pose ? (
          <>
            ...
            <div className="h-1.5 bg-bg rounded-full overflow-hidden border border-line mb-4 max-w-[440px] mx-auto">
              <div className="h-full bg-brass transition-all" style={{ width: `${(seqIndex / sequence.length) * 100}%` }} />
            </div>
            <div className="bg-surface border border-line rounded-[10px] p-8 text-center">
              ...
              <div className="text-2xl font-['Oswald'] uppercase tracking-wider text-bone mt-2.5">{pose.name}</div>
              ...
              <div className="text-sm text-bone-dim max-w-[440px] mx-auto mt-3.5 leading-relaxed">{pose.cue}</div>
            </div>
            <div className="flex gap-2.5 justify-center mt-5 flex-wrap">
              <button onClick={() => goToPose(seqIndex - 1)}
              ...
              <button onClick={() => setSeqRunning((r) => !r)}
                ...
                {seqRunning ? <Pause className="w-3.5 h-3.5 inline mr-1" /> : <Play className="w-3.5 h-3.5 inline mr-1" />}
              ...
              <button onClick={() => { setInSequence(false); setSeqRunning(false); speechSynthesis.cancel(); if (seqIntervalRef.current) clearInterval(seqIntervalRef.current); }}
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-bone-dim">Nice work &mdash; that's the full 26-posture flow. Rest in Savasana as long as you like.</p>
            <button onClick={() => setInSequence(false)}
    ...
    <>
      <div className="flex justify-between items-end flex-wrap gap-3 mb-7">
        <div>
          ...
          <div className="text-sm text-bone-dim mt-1.5 max-w-[520px]">
            50 poses across standing, backbend, forward fold, twist, balance, hip-opener, core and restorative.
          </div>
        </div>
        <button
          onClick={() => { setInSequence(true); setSeqIndex(0); setSeqRemaining(sequence[0]?.hold || 30); }}
          className="bg-brass text-[#1a1408] border-none px-4 py-2.5 rounded-md font-semibold text-xs uppercase tracking-[0.03em] hover:bg-[#d69a4e]"
        >
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(190px,1fr))] gap-3.5">
        {filtered.map((pose, i) => (
              ...
              openModal(
                <div className="text-center">
                  ...
                  <div className="flex justify-center gap-1.5 mb-3 flex-wrap">
                    <span className="text-[10px] uppercase tracking-[0.05em] px-2 py-0.5 rounded-full border border-line text-bone-dim">{pose.category}</span>
                    <span className={`text-[10px] uppercase tracking-[0.04em] px-2 py-0.5 rounded-full border
                    ...
                    <span className="text-[10px] font-['IBM_Plex_Mono'] px-2 py-0.5 rounded-full border border-line text-bone-dim">~{pose.hold}s hold</span>
                  </div>
                  <p className="text-sm text-bone-dim leading-relaxed">{pose.cue}</p>
                  <p className="text-xs text-bone-dim mt-2"><b className="text-bone">Focus:</b> {pose.focus}</p>
                </div>
              );
            }}
          ...
          >
            {pose.seq && (
              <div className="absolute top-2.5 left-2.5 font-['IBM_Plex_Mono'] text-[10px] text-brass-dim">#{pose.seq}</div>
            )}
        ...
        {filtered.length === 0 && (
          <div className="col-span-full text-center text-bone-dim text-sm py-5">No poses in this category.</div>
        )}
      </div>
    </>
  );
...
function NutritionView({
    ...
    return (
      <div className="mb-3">
        <div className="flex justify-between text-xs text-bone-dim mb-1">
          ...
          <span className="font-['IBM_Plex_Mono']">{Math.round(current)} / {target}</span>
        </div>
        <div className="h-2 bg-bg rounded-full overflow-hidden border border-line">
  ...
  return (
    <>
      <div className="mb-7">
        <h1 className="text-[30px] font-['Oswald'] uppercase tracking-wider text-bone">Nutrition</h1>
        <div className="text-sm text-bone-dim mt-1.5">Track macros, browse foods, and plan your meals.</div>
      </div>
      ...
      {nutView === "dashboard" && (
        <>
          <div className="bg-surface border border-line rounded-[10px] p-5 mb-4">
            ...
            <MacroBar label="Protein" current={totals.protein} target={goals.protein} color="#7ea366" />
          ...
          {mealTypes.map((mt) => {
            ...
            const mealFoods = mealLog
              ? mealLog.foodIds.map((fid) => nutritionState.foods.find((f) => f.id === fid)).filter(Boolean) as FoodItem[]
              : [];
            return (
      ...
      {nutView === "foods" && (
        <FoodLibraryView foods={nutritionState.foods} />
      )}
...
function FoodLibraryView({ foods }: { foods: FoodItem[] }) {
  ...
  return (
    <>
      ...
      <div className="bg-surface border border-line rounded-[10px] p-5">
        {filtered.map((f) => (
          <div key={f.id} className="flex justify-between items-center gap-3 py-3.5 border-b border-line last:border-b-0">
            <div className="min-w-0">
              <div className="font-semibold text-sm text-bone">
                {f.name}
...
function MealPlannerView({
  mealSuggestions, foods, nutritionState, updateNutrition,
}: {
    ...
    <>
          ...
          {mealSuggestions.map((m) => (
            <div key={m.id} className="bg-surface border border-line rounded-[10px] p-3.5">
              <div className="flex justify-between items-start">
                ...
                <span className={`text-[10px] uppercase tracking-[0.05em] px-2 py-0.5 rounded-full border shrink-0 ml-2
      ...
      <div className="bg-surface border border-line rounded-[10px] p-5">
        ...
        {mealTypes.map((mt) => {
          ...
          const mealFoods = log
            ? log.foodIds.map((fid) => foods.find((f) => f.id === fid)).filter(Boolean) as FoodItem[]
            : [];
          return (
            <div key={mt} className="flex items-center gap-3 py-2.5 border-b border-line last:border-b-0">
              <div className="w-20 shrink-0">
                <span className="text-[11px] uppercase tracking-[0.05em] text-brass">{mt}</span>
              </div>
              <div className="flex-1">
                {mealFoods.length > 0 ? (
                  <div className="text-sm text-bone-dim">{mealFoods.map((f) => f.name).join(", ")}</div>
                ) : (
        ...
        <div className="flex justify-between items-center pt-3 mt-1 border-t border-line">
          ...
          <span className="text-sm font-['IBM_Plex_Mono'] text-brass">
            {Math.round(dayTotals.kcal)}kcal &middot; {Math.round(dayTotals.protein)}g P &middot; {Math.round(dayTotals.carbs)}g C &middot; {Math.round(dayTotals.fat)}g F
          </span>
        </div>
      </div>
    </>
